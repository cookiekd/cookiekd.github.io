...
(function(window, $, undefined) {
    ...
    function Grid(element, config) {
        ...
        this._init(config);
        this.render();
        this._autoIndex();	// create custom by leedagam
    }
    ...
    Grid.prototype = {
        ...
        loadData: function(filter) {
            ...
            var rtn = this._controllerCall("loadData", filter, args.cancel, function(loadedData) {
                if(!loadedData)
                    return;
        ...
        insertItem: function(item) {
			...
			insertingItem['ROW_TYPE']= "1";
			insertingItem['DATA_TYPE']= "NEW";
            ...
            var rtn = this._controllerCall("insertItem", insertingItem, args.cancel, function(insertedItem) {
        ...
        _updateRow: function($updatingRow, editedItem) {
			...
			if(updatedItem['DATA_TYPE'] != 'NEW'){
				updatingItem['ROW_TYPE']= "2";
				updatedItem['ROW_TYPE']= "2";
			}
        ...
        _deleteRow: function($row) {
			...
			if(deletingItem['DATA_TYPE'] != 'NEW'){
				deletingItem['ROW_TYPE']= "3";
			}
            ...
            var loadStr = this.loadStrategy();
            var fields = loadStr._grid.fields;
            var autoIdx = false;
			var autoIndexFeild;	// autoindex name
            for(var i in fields){
        		var fIdx = fields[i];
        		if(fIdx.autoIndex){
        			autoIdx = fIdx.autoIndex;	//true / false
        			autoIndexFeild = fIdx.name;
        			break;
        		}
        	}
            if(autoIdx){
            	var data = this.data;
            	var thisDeleteItem = deletingItem[autoIndexFeild];	
            	var dataIdxField;
            	for(var i in data){
            		dataIdxField = data[i][autoIndexFeild];
            		if(dataIdxField > thisDeleteItem){
            			data[i][autoIndexFeild] = Number(dataIdxField) - 1;
            		}
            	}
            	this.data = data
            }
        ...
        _autoIndex: function(){
        	var loadStr = this.loadStrategy();
        	var datas = loadStr._grid.data;
        	var fields = loadStr._grid.fields;
        	var container = loadStr._grid._container[0];
			var autoIdx = false;
			var autoIndexFeild;	// autoindex name
			var idx;
        	for(var i in fields){
        		...
        		if(fIdx.autoIndex){
        			...
        			idx = i;
        			for(var j in datas){
        				if(!datas[j].hasOwnProperty(autoIndexFeild)){
        					this.data[j][autoIndexFeild] = Number(j)+1;
        				}
        			}
        			break;
        		}
        	...
        	this.reset();
            if(autoIdx){
            	var basicGrid = $(container);
            	var insertModeBtn = basicGrid.find("th.jsgrid-header-cell").find("input.jsgrid-insert-mode-button");
            	
            	insertModeBtn.click(function(){
            		if(insertModeBtn.hasClass("jsgrid-mode-on-button")){
            			var insertIdx = 0;
            			var arrTr = basicGrid.find("div.jsgrid-grid-body").find("tbody").find("tr");
            			if(!arrTr.hasClass("jsgrid-nodata-row")){
            				insertIdx = arrTr.length;
            			}
            			
            			var gridHeader = basicGrid.find("div.jsgrid-grid-header").find("tr.jsgrid-insert-row");
            			var insertBtn = gridHeader.attr("style");
            			gridHeader.find("td:eq(" + idx + ") > input").val(Number(insertIdx)+1);
            		}
            		basicGrid.find("div.jsgrid-grid-header").find("tr.jsgrid-insert-row").find("input.jsgrid-insert-button").click(function(){
            			var display = basicGrid.find("div.jsgrid-grid-header").find("tr.jsgrid-insert-row").attr("style");
            			if(typeof display == "undefined"){
            				insertModeBtn.click();
            				basicGrid.find("div.jsgrid-grid-header").find("tr.jsgrid-insert-row").css("style","display: none;");
            			}
            		});
            		return;
            	});
